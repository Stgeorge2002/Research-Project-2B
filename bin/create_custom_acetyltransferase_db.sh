...
esearch_result=$(curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=nucleotide&term=${ENCODED_TERM}&usehistory=y&retmax=500")
...
# Fetch the sequences
curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nucleotide&WebEnv=${WEBENV}&query_key=${QUERYKEY}&rettype=fasta_cds_na&retmode=text&retmax=500" > raw_sequences.fasta
